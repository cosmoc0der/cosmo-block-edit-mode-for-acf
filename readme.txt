...
Tags: block editor, gutenberg, custom fields, blocks, editor
...
The plugin builds a patched copy of the ACF blocks script on the fly. The only change is one internal "is the editor iframed?" check, which is made to always return false. The copy is stored in `wp-content/uploads/block-edit-mode-for-acf/` and its file name is derived from the ACF version and the source file, so it is rebuilt automatically after every ACF update.
...
3. Activate **Block Edit Mode for ACF**.
